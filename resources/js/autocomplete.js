...
function initAutocomplete() {
    ...
    autocomplete = new window.google.maps.places.Autocomplete(input, {
        types: ['address'],
        componentRestrictions: { country: [] }, // Add country code(s) if needed
    });
...
function onPlaceChanged() {
    ...
    components.forEach((component) => {
        ...
        if (types.includes('administrative_area_level_1')) document.getElementById('stateInput').value = component.long_name;
        if (types.includes('postal_code')) document.getElementById('postalCodeInput').value = component.long_name;
