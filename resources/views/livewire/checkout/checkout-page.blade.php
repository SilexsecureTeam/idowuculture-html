            ...
            <div class="flex-1 px-1">
                ...
                <form wire:submit.prevent="placeOrder" class="grid grid-cols-2 gap-2">
                    ...
                    <div class="col-span-2">
                        ...
                        <input wire:model="name" readonly type="text" id="name" class="w-full border border-gray-300 px-3 py-2 focus:outline-none focus:ring-2 focus:ring-yellow-900 transition" placeholder="Full Name">
                        @error('name') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
                    </div>

                    <div class="col-span-2">
                        <label for="email" class="mb-1 text-sm font-medium text-gray-700">Email</label>
                        <input wire:model="email" readonly type="email" id="email" class="w-full border border-gray-300 px-3 py-2 focus:outline-none focus:ring-2 focus:ring-yellow-900 transition" placeholder="Email">
                        @error('email') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
                    </div>

                    <div class="col-span-2">
                        <label for="phone" class="mb-1 text-sm font-medium text-gray-700">Phone</label>
                        <input wire:model="phone" readonly type="text" id="phone" class="w-full border border-gray-300 px-3 py-2 focus:outline-none focus:ring-2 focus:ring-yellow-900 transition" placeholder="Phone">
                        @error('phone') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
                    </div>

                    <div class="col-span-2">
                        <label for="address" class="mb-1 text-sm font-medium text-gray-700">Address</label>
                        <textarea wire:model="address" id="addressInput" class="w-full border border-gray-300 px-3 py-2 focus:outline-none focus:ring-2 focus:ring-yellow-900 transition" placeholder="Address"></textarea>
                        @error('address') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
                    </div>

                    <div class="w-full mt-4 col-span-2">
                        <button type="submit" class="w-full cursor-pointer bg-yellow-900 px-6 text-white py-3 font-semibold hover:bg-yellow-800 transition">Place Order</button>
                    </div>
                </form>
            </div>
        ...
        <div class="bg-black w-full text-white md:p-8 py-5 my-8 rounded-xl flex items-center justify-between">
            <div class="flex w-full flex-col md:flex-row items-center justify-between gap-4 px-4">
                ...
                <form class="flex flex-col gap-2 max-w-[350px] w-full">
                    <div class="relative">
                        <input type="email" placeholder="Enter your email address" class="px-4 pl-8 py-3 rounded-3xl bg-white max-w-[350px] w-full outline-none text-black flex-1">
